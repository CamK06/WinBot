...
namespace WinBot.Commands.Fun
{
    ...
    {
		[Command("horny")]
        [Description("Tell someone (Revvy) to stop being horny.")]
        [Usage("[User] [Image: parz, agp, agp2, mehdi]")]
        [Category(Category.Fun)]
        public async Task horny(CommandContext Context, string screenname = "", string image = "parz")
        {
            ...
            bmp.DrawString("STOP BEING HORNY", IMPACTfont, brush, genX, bedY, snFormat);
			bmp.DrawString(screenname.ToUpper(), IMPACTfont, brush, genX, userY, snFormat);
			
			// Save the image to a temporary file
            bmp.Save();
            string imagePath = TempManager.GetTempFile($"horny-{image}-{screenname}-{Context.User.Id}.png", true);
            img.Save(imagePath);
            await Context.Channel.SendFileAsync(imagePath);
            TempManager.RemoveTempFile($"horny-{image}-{screenname}-{Context.User.Id}.png");
		}
