...
namespace WinWorldBot.Commands
{
    public class DataReqCommand : ModuleBase<SocketCommandContext>
    {
        ...
        private async Task RequestData()
        {
            ...
            if(File.Exists($"DataRequests/{Context.Message.Author.Username}-{DateTime.Now.ToShortDateString()}.json")) goto send;
            ...
            send:
            if(File.ReadAllBytes($"DataRequests/{Context.Message.Author.Username}-{DateTime.Now.ToShortDateString()}.json").Length > 7500000)
            {
            ...
            // Send the data
            await Context.Message.Author.SendFileAsync($"DataRequests/{Context.Message.Author.Username}-{DateTime.Now.ToShortDateString()}.json", "Here's your data!");
