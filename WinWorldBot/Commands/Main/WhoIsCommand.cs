...
namespace WinWorldBot.Commands
{
    public class WhoIsCommand : ModuleBase<SocketCommandContext>
    {
        ...
        private async Task WhoIs(SocketGuildUser user)
        {
            ...
            if(user.Activity.Type == ActivityType.Listening) Embed.AddField("**Listening**", user.Activity.Details, true);
            else if(user.Activity.Type == ActivityType.Playing) Embed.AddField("**Playing**", user.Activity.Details, true);
            else if(user.Activity.Type == ActivityType.Watching) Embed.AddField("**Watching**", user.Activity.Details, true);
            else if(user.Activity.Type == ActivityType.Streaming) Embed.AddField("**Streaming**", user.Activity.Details, true);
            else if(user.Activity.Type == ActivityType.CustomStatus) Embed.AddField("**Status**", user.Activity.Details, true);
